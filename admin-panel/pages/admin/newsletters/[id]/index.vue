      ...
      <div v-else-if="newsletter" class="space-y-6">
        ...
        <div class="bg-card rounded-lg shadow p-6">
          <div class="flex justify-between items-start mb-4">
            <div>
              <h1 class="text-2xl font-bold text-foreground mb-2">{{ newsletter.title }}</h1>
              <div class="flex items-center space-x-4 text-sm text-muted-foreground">
                ...
                <span v-if="newsletter.published_at">발행: {{ formatDate(newsletter.published_at) }}</span>
              </div>
            </div>
          </div>
        </div>
...
// 메타 태그
useHead({
  title: () => newsletter.value ? `${newsletter.value.title} - 뉴스레터` : '뉴스레터 보기',
  meta: [
    { name: 'description', content: '뉴스레터 상세 보기' }
  ],
})
</script>
